...
NO_HINT_PROMPT = "Write a radiology report for this chest X-ray."
...
def build_input_prompts(disease_labels: torch.Tensor = None, use_hint: bool = True) -> List[str]:
    """Build prompts from disease labels only — no retrieval text injected.
    When use_hint=False (A3 ablation), returns a fixed no-hint prompt."""
    if not use_hint:
        n = disease_labels.size(0) if disease_labels is not None else 1
        return [NO_HINT_PROMPT] * n
    prompts = []
...
class DiseaseT5(nn.Module):
    ...
    def forward(self, frontal, lateral, target_reports,
        ...
        use_hint = getattr(self.train_config, "use_disease_hint", True)
        use_cls  = getattr(self.train_config, "use_cls_loss", True)
        prompts = build_input_prompts(hint_labels, use_hint=use_hint)

        # Auxiliary classification loss -- pooled from perceiver visual tokens.
        if use_cls and training and disease_labels is not None and visual_tokens is not None:
    ...
    def generate(self, frontal, lateral,
        ...
        if not use_hint:
            # A3: no disease hint channel -- fixed prompt
            prompts = [NO_HINT_PROMPT] * frontal.size(0)
        else:
            if gt_labels is not None:
                # Oracle: GT findings -> hint (isolates decoder from predictor)
                disease_labels = gt_labels.float().to(device)
            elif not use_cls:
                # A4: disease_head untrained -> fall back to xrv-only
                xrv_probs = self._xrv_disease_probs(frontal, lateral)
                disease_labels = (xrv_probs > self._xrv_thresholds.to(device)).float()
            else:
