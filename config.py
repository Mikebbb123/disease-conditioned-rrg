...
@dataclass
class TrainConfig:
    output_dir: str = "/content/drive/MyDrive/iu_xray_rag/output_resampler_rag"
    ...
    # Visual tokens — disabled (ablation showed harmful with leaky prompt).
    # Re-enable later with zero-init gate once findings→report path is stable.
    use_visual_tokens: bool = False

    # ---- Ablation switches ----
    use_disease_hint: bool = True   # A3: False → remove disease hints from prompt
    use_cls_loss: bool = True       # A4: False → remove auxiliary classification loss
